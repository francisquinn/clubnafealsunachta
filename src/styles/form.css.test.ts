...
describe("form.css mobile textarea styles", () => {
  ...
  it("has adequate min-height for textarea on mobile", () => {
    // Should have a min-height for textarea to ensure adequate height
    expect(formCss).toMatch(/textarea\.cnf-form__input\s*\{[^}]*min-height/);
  });
